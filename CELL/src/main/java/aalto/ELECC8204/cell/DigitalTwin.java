...
public class DigitalTwin {
    ...
    private ArrayList<LegoConnection> connections = new ArrayList<LegoConnection>();  
    ...
    ArrayList<DigitalPart> legos = new ArrayList<DigitalPart>(100);
    ArrayList<DigitalPart> unassembledLegos = new ArrayList<DigitalPart>(100);
...
//        RobotArm.step = 0.1f;            
        for (int i=0; i<legosSize; i++) {
            if(legos.get(i).id.equals(id)) {
                ...
                legos.get(i).node.rotate(-1*legos.get(i).orientation.getX(), -1*legos.get(i).orientation.getY(), -1*legos.get(i).orientation.getZ());
//                legos.get(i).translate(legos.get(i).startLocation);
            }
/*            if(legos.get(i).northSouthOrientation()) {
                legos.get(i).node.rotate(0, -1*FastMath.HALF_PI, 0);
    ...
    public DigitalTwin(List<DigitalPartType> parts, Set<UaReference> conns, AssetManager assetManager, Cell c, TuotekuvausClient uaClient) throws ServiceException, AddressSpaceException {
        cell = c;
        ...
        for(DigitalPartType part : parts) {
        	if(uaClient.compareType(part, aalto.ELECC8204.codegen.DigitalProductDescription.Ids.SquareLegoType)) {
        	...
        	else if(uaClient.compareType(part, aalto.ELECC8204.codegen.DigitalProductDescription.Ids.RectangleLegoType)) {
        ...
        /*
         * This is here only for CAD-parts to work properly.
         * It gets all cadTypes from the address space
         * and calls add3dparam once for each connectionPoint of each type.
         * 
        ...
        for(UaNode type : uaClient.getCadTypes()) {
        	...
        	for(CoordinateType connectionPoint : uaClient.getConnectionPoints(type)) {
        		...
        		String coordinates = connectionPoint.getX() + "," + connectionPoint.getY() + "," + connectionPoint.getZ();
        		CADpart.add3dParam(typename + name, coordinates);
        	}
        }
        
        /*
         * Iterating through all OPC UA references between connectionPoint
         * and creating corresponding Connection-object.
         */
        ...
        Iterator<Entry<String, DigitalPart>> it = map.entrySet().iterator();
        DigitalPart lego;
        int index = 0;
        while (it.hasNext()) {
        	Entry<String, DigitalPart> pair = it.next();
            lego = (DigitalPart) pair.getValue();
//            lego.startLocation = new Vector3f(6.0f - 12.0f * (index%2), assemblySurfaceHeight + Main.dim, -4.0f - 2.0f * (index/2));
...
//            it.remove(); // avoids a ConcurrentModificationException
        ...
        // Now go through the lego connections and translate each lego into the correct place
        // We iterate until we find an unconnected connection involving one lego that was already assembled
        // With this tactic, we have to iterate over the connections several times until everything is connected
...
//                    System.out.println(connection.lego2.targetLocation.toString());
                    // Now translate lego2 so that its appropriate connectionPoint is being connected
...
//                    System.out.println(connection.lego2.targetLocation.toString());
                ...
                if (!connection.lego1.assembled && connection.lego2.assembled) {
                    ...
                    // Now translate lego1 so that its appropriate connectionPoint is being connected
    ...
    public DigitalTwin(String fileName, AssetManager assetManager, Cell c) {
        ...
        try {
            InputStream inputStream = DigitalTwin.class.getResourceAsStream(fileName);
...
/*                    if (words[1].equals("faceplateback")|| words[1].equals("FaceplateBack")) {
                        FaceplateBack f = new FaceplateBack(words[2], assetManager);
                        f.twin = this;
                        map.put(f.id,f);
                        if (firstPart) {
                            firstPart = false;
                            f.translateTarget(new Vector3f(-7,0,-10));
                            f.assembled = true;
                            f.assemblyLocation = f.targetLocation;
                        }
                        node.attachChild(f.node);                        
                        legos.add(f);
                    }
                    if (words[1].equals("boltangular")|| words[1].equals("BoltAngular")) {
                        BoltAngular b = new BoltAngular(words[2], assetManager);
                        b.twin = this;
                        map.put(b.id,b);
                        if (firstPart) {
                            firstPart = false;
                            b.translateTarget(new Vector3f(-7,0,-10));
                            b.assembled = true;
                            b.assemblyLocation = b.targetLocation;
                        }
                        node.attachChild(b.node);                        
                        legos.add(b);
                    }                    
                    if (words[1].equals("bolt")|| words[1].equals("Bolt")) {
                        Bolt b = new Bolt(words[2], assetManager);
                        b.twin = this;
                        map.put(b.id,b);
                        if (firstPart) {
                            firstPart = false;
                    ...
                    if (words[1].equals("shaft")|| words[1].equals("Shaft")) {
                    ...
                    if (words[1].equals("pendulum")|| words[1].equals("Pendulum")) {
                        Pendulum p = new Pendulum(words[2], assetManager, words[3], words[4]);
                        p.twin = this;
                        map.put(p.id,p);
                        if (firstPart) {
                            firstPart = false;
...
//                    System.out.println(connection.lego2.targetLocation.toString());
    ...
    // goes through all elements in list "legos" and puts unassembled legos into the list "unassembledLegos"
    // if they have a connection to an assembled lego
    // then it goes through the list again and returns the one with the lowest "y" assembly location
    public DigitalPart nextUnassembledLego() {
...
//                RobotArm.maxRotationStep = 500;
            ...
            if (lego.approachAttempt == 1) {  // towards negative X axis
                ...
                trajectory.addPoint(new Vector3f(lego.assemblyLocation.add(new Vector3f(20f*Main.dim, -4.5f*Main.dim, 0))));                    
                trajectory.addPoint(new Vector3f(lego.assemblyLocation.add(new Vector3f(0, -4.5f*Main.dim, 0))));                    
                trajectory.addPoint(new Vector3f(lego.assemblyLocation.add(new Vector3f(0, 0f*Main.dim, 0)))); // used to be 2f instead of 0f for y coordinate
                ...
                trajectory.addPoint(new Vector3f(lego.assemblyLocation.add(new Vector3f(0, -4.5f*Main.dim, -5f*Main.dim))));                    
                trajectory.addPoint(new Vector3f(lego.assemblyLocation.add(new Vector3f(0, -4.5f*Main.dim, 0))));                    
                trajectory.addPoint(new Vector3f(lego.assemblyLocation.add(new Vector3f(0, 0f*Main.dim, 0))));
                if(physicalAssembly) {
                    lego.node.rotate(FastMath.PI, 0, 0);                                     
...
/*    public boolean virtualAssemblyComplete() {
        return virtualAssemblyComplete;
    }
  */  
...
/*                    if(lego.orientation != null) {
                        if(lego.orientation.getZ() > 0.1f) {
                            ...
                            lego.node.setLocalTranslation(-1f*lego.getXextent(), -3*Main.dim, 1f*lego.getYextent());
                        }
                    } else {*/
                        lego.node.setLocalTranslation(0, -3.0f * Main.dim - lego.getYextent(), 0);                        
//                    }
...
//                        Main.helloText.setText(lego.assemblyLocation.toString() + " : " + lego.targetLocation.toString());
                    }
...
/*                Vector3f v1 = new Vector3f(lego.startLocation);
        ...
        Main.markerGeom.setLocalTranslation(lego.startLocation.add(lego.getGripperOffset()));                    
                    ...
                    v2.setY(maxHeight);
                    trajectory.addPoint(v2);
//                    trajectory.addPoint(new Vector3f(lego.assemblyLocation.add(new Vector3f(0, 0.5f*Main.dim, 0))));
                    trajectory.addPoint(lego.assemblyLocation.add(lego.getGripperOffset()));
                    trajectory.initTrajectory();
                    trajectoryMotion = true;
                    if(physicalAssembly && (lego.state != LegoState.AtAssembly)) {
